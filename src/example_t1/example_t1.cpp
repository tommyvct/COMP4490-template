...
//
//  Simple tessellation shader example, with a single triangle.
//
...
const char *WINDOW_TITLE = "Triangle Tessellation, Flat Shading";
...
init()
{
    ...
    GLuint program = InitShader( "vshader_t1.glsl", "fshader_t1.glsl", "tcs_t1.glsl", "tes_t1.glsl" );
...
display( void )
{
    ...
    // The code below draws the geometry twice.

    // The first time, it is filled (Outline is set to zero).

    // The second time, it is an outline (Outline is set to one). The outline
    // is offset slightly from the filled vertices so that it appears "on top".
    // In order to easily draw the outline it uses GL_LINE glPolygonMode, which
    // causes the primitives to be outlined instead of filled.
    ...
    glUniformMatrix4fv( ModelView, 1, GL_FALSE, glm::value_ptr(model_view * glm::translate(glm::mat4(), glm::vec3(0, 0, 0.002))) );
